...
export default function BookingsDashboardView() {
  // Active Category State Tabs Tracker
  const [activeTab, setActiveTab] = useState<"upcoming" | "past" | "cancelled">("upcoming");
  
  // Storage arrays for bookings rows mapping data
  ...
  // Independent dropdown state locks mapping for the "•••" menu
  const [openMenuId, setOpenMenuId] = useState<string | null>(null);
  const menuRef = useRef<HTMLDivElement | null>(null);

  // ✅ CRITICAL IDENTITY: Matching your exact local database user row item
  const MASTER_USER_ID = "63d7eb95-bc65-4f38-89c5-a6e54fca64f0";

  // Core network data engine handler
  const fetchDashboardRegistry = async () => {
    try {
      setIsLoading(true);
      setErrorMsg("");

      // ✅ PATH MATCH FIXED: Hitting the exact endpoint that houses your GET and PATCH route
      const response = await fetch(`/api/bookings?userId=${MASTER_USER_ID}`);
      const payload = await response.json();
      
      if (payload.success && payload.data) {
        // Hydrating state fields directly using data parsed out of the database API arrays
        setBookingsData({
      ...
      console.error("Network runtime failed reading bookings index:", err);
  ...
  // ✅ AUTO MOUNT: Triggers data ingestion loop immediately as soon as the dashboard screen paints
  useEffect(() => {
    fetchDashboardRegistry();
  }, []); 

  // Barrier layer to close open contextual dropdown frames automatically on side clicks
  useEffect(() => {
  ...
  const handleCancelActionCall = async (bookingId: string) => {
    ...
    if (cancellationNote === null) return; // Sequence aborted safely by host

    try {
      // ✅ PATH MATCH FIXED: Calling the exact endpoint that houses your PATCH handler
      ...
      if (response.ok && result.success) {
        setOpenMenuId(null);
        // Instant hot reload sync to reposition updated states across views smoothly
  ...
  // Humanize time calculations directly inside list row instances
  ...
  return (
    <div className="space-y-6 max-w-[960px] mx-auto select-none text-left">
      ...
      {/* SECTION TOP HEADER BRAND TILES */}
      ...
      {/* FILTER BUTTON TABS SELECTOR PANEL CONTAINER */}
      ...
      {/* COMPONENT HOUSING VIEWS CONDITIONS GRID SWITCH */}
      {isLoading ? (
        <div className="py-20 text-center text-xs text-zinc-500 font-mono animate-pulse">
          Loading bookings data registry streams...
        </div>
      ) : errorMsg ? (
          ...
          {currentActiveList.map((booking) => (
            <div key={booking.id} className="p-5 flex items-start justify-between hover:bg-[#18181b]/30 transition-all group relative">
              ...
              <div className="flex items-start space-x-6">
                {/* DATE & TIME DETAILS CARD SEGMENT */}
                <div className="w-[110px] space-y-0.5 shrink-0 pt-0.5">
                  ...
                  <a
                    href="#"
                    onClick={(e) => { e.preventDefault(); alert("Initializing Cal Video integration layers..."); }}
                ...
                {/* VISUAL CORE BOOKER SUBJECT DETAILS SECTION ROWS BLOCK */}
                <div className="space-y-1">
                  ...
                  <div className="text-[11px] text-zinc-500 font-light space-y-0.5">
                    ...
                    {/* Dynamic notes tracking extractor */}
              ...
              {/* ACTION THREE-DOTS PANEL DROPDOWN POPUPS CONTROLS */}
              {activeTab === "upcoming" && (
                <div className="relative">
                  <button
                    onClick={() => setOpenMenuId(openMenuId === booking.id ? null : booking.id)}
                    className="w-7 h-7 flex items-center justify-center rounded-lg border border-zinc-800 hover:bg-zinc-800 text-zinc-400 hover:text-white transition-all cursor-pointer text-xs font-bold"
                  >
                    •••
                  </button>
                  ...
                  {openMenuId === booking.id && (
                    <div
                      ref={menuRef}
                      className="absolute right-0 mt-1 w-44 bg-[#1c1c1e] border border-zinc-800 rounded-lg shadow-2xl z-50 py-1 font-medium text-xs text-left animate-in fade-in duration-70"
                    >
                      <button
                        onClick={() => { alert(`Rescheduling session references for id: ${booking.id}`); setOpenMenuId(null); }}
                        className="w-full px-3 py-2 text-left text-zinc-300 hover:bg-zinc-800 hover:text-white transition-all cursor-pointer block"
                      >
                        Reschedule booking
                      </button>
                      <button
                        onClick={() => handleCancelActionCall(booking.id)}
                        className="w-full px-3 py-2 text-left text-red-400 hover:bg-red-500/10 transition-all cursor-pointer block border-t border-zinc-900/80 font-semibold"
                      >
                        Cancel event
                      </button>
                    </div>
              ...
              {/* TAB LABEL BADGES FLAGS LAYOUT DISPLAY */}
              {activeTab === "past" && (
